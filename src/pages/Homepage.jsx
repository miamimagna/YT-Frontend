        ...
        (async () => {
            try {
                if (data.carousel.length !== 0) {
                    ...
                    const { data: { trendingRes, bollyRes, popRes } } = await axios.get('https://localhost:5000/homepage');
        ...
        (async () => {
            try {
                if (optionData.commute.length !== 0 || optionData.relax.length !== 0 || optionData.energize.length !== 0 || optionData.workout.length !== 0 || optionData.focus.length !== 0) {
                    ...
                    const { data: { commuteRes, relaxRes, energizeRes, workoutRes, focusRes, } } = await axios.get('https://localhost:5000/option');
